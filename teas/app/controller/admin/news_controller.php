...
/**
 * Controller_Admin_News 控制器
 */
class Controller_Admin_News extends Controller_Abstract
{
  /**
   * 业界动态列表
   *
   */
	function actionIndex()
	{
		...
		$form=new QForm('newsform',url('news/update'));
		
		//是否是查询新闻
		if(empty($this->_context->find)){
			
					$select= News::find()->order('order_num DESC');
					$agrs =null;
...
//			}
			...
			$select =News::find($condition)->order('order_num DESC');
			$agrs =array('sorts'=>$sortId,'keywords'=>$title,'find'=>1);
		}
		
	  	$select->limitPage($page,20);
	  	
	  	$this->_view['page']=$select->getPagination();
	  
	   	$this->_view['news']=$select->query('newsort,user');
	   	
	  	$this->_view['form']=$form;
	  	//构建查询表单
	  	$findForm= new QForm('new_find',url('news/index'));
	  	$findForm->add(QForm::ELEMENT ,'keywords',array('_ui'=>'textbox','_label'=>'新闻标题'))
	...
	function actionEdit(){
		...
		$form=Form_Admin_News::createForm('news',url('news/edit'));
		$newId=$this->_context->id;
		$news= News::find('id =?',$newId)->query();
		//数组导入到表单
		$form->import($news);
		...
		if($this->_context->isPOST() && $form->validate($_POST)){
			try {
				...
				$news->save();
				return $this->_redirectMessage('编辑文章内容成功','如果你不做出选择系统将自动跳转',url('news/index'),3);
...
//				$news['newsort']['news_count']=
	...
	function actionUpdate(){
		...
		//是否要选择了删除复选框
		if(!empty($newsid)){
			News::meta()->deleteWhere('id in(?)',$newsid);
		}
		//的到要更新的列表
	    ...
	    foreach  ($ordernum as $id=>$ordernum){
			...
			News::meta()->updateWhere(array('order_num'=>$ordernum),'id =?',$id);
		}
		return $this->_redirectMessage('更新文章信息成功','如果你不做出选择系统将自动跳转',url('news/index'),3);
	}
	/**
	 * ajax方式改变新闻是否热点
	 *
	 */
	 function actionAjaxIsHot(){
	  ...
	  if($this->_context->isAJAX()){
	 	...
	 	$news_id =$this->_context->news_id;
	  	//更新用户的数据
	    News::meta()->updateDbWhere(array('is_hot'=>$is_hot),"id =?",$news_id);
	 	echo 1;
	  }else {
	 ...
	 function actionAjaxIsTop(){
	 	...
	 	if($this->_context->isAJAX()){
	 		...
	 		//更新新闻是否推荐
	 		try {
	 			News::meta()->updateWhere(array('recommend'=>$is_top),'id =?',$news_id);
	 			echo true;
